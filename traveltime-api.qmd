...
sample_isochrone |> 
  inner_join(venues, by = "venue") |> 
    ggplot()+
  # Map of LA tracts
    geom_sf(data = lapop2020_shp_subset, aes(geometry = geometry, fill = POPULATION), colour = NA)+
...
```{r}
list_rbind(venues_isochrones_full)
...
map_isochrones = function(data){
  ...
  facet_grid(rows = vars(arrival_time), cols = vars(transportation), 
             labeller = labeller(arrival_time = stamp_time("8am on Tuesday"), transportation = str_to_title))+
    theme_void()
}

venues_isochrones_df |> 
  split(venues_isochrones_df$venue) |> 
  map(map_isochrones)
```
...
lapop2020_shp_subset |> 
  ...
  geom_sf(data = list_rbind(venues_isochrones, names_to = "id"), aes(geometry = geometry), fill = "red", alpha = 0.5)+
  scale_fill_continuous(low = "white", high = "blue")+
  theme_void()
```
...
```{r}
venues_iso_locations = venues_iso |> 
  mutate(venue_locations = make_location(id = venue, coords = list(lat = latitude, lng = longitude)))
```
...
sample_func <- function(method = "public_transport", venue, coords, time_period = "weekday_morning"){
  
  make_search(
    id = str_c(method, " to ", venue),
    travel_time = 900,
    coords = list(lat = coords[2], lng = coords[1]),
    arrival_time_period = "weekday_morning",
