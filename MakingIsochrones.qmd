...
What's the population and area of this subset?
...
```{r}
twentyeight_folder = here("data", "TwentyEightBy28")

twentyeight_df = twentyeight_folder |>
  fs::dir_ls(recurse = TRUE, regexp = 'shp$') |>
  map(st_read) |> 
  map(st_transform, crs = "EPSG:4326") |>
  list_rbind()
...
```{r}
measurem_folder = here("data", "MeasureM")

measurem_df = measurem_folder |>
  fs::dir_ls(recurse = TRUE, regexp = 'shp$') |>
  map(st_read) |> 
  map(st_transform, crs = "EPSG:4326") |>
  list_rbind()
...
measurem_df |> 
  ggplot()+
  geom_sf(aes(geometry = geometry, colour=Mode2))+
  theme_minimal()
```
...
venues_iso_search = iso_request_df |> 
    pmap(\(id, coords, travel_time, date, time_of_day) make_search(id = str_c(id, "_", travel_time), 
                                      travel_time = travel_time, 
                                      arrival_time = next_weekday(date, time_in_hrs = time_of_day), 
                                      coords = coords, 
...
```{r}
tictoc::tic("API requests for Transit")
...
isochrones_transit_venues = isochrones_transit_df |> 
  # filter(venue == sample(venue, 1)) |> 
  # inner_join(venue_addresses, by = "venue") |> 
  # distinct(id, venue_location, travel_time, arrival_time, transportation, walking_time, pt_change_delay, group, geometry) |> 
...
sf::st_as_sf(isochrones_transit_df) |> 
  sf::st_write(here("data", str_c("isochronesTransit", today(), ".geojson.json")), driver = "GeoJSON")
...
isochrone_plots = st_as_sf(isochrones_transit_venues) |> 
  mutate(time = hour(ymd_hms(arrival_time, tz = "America/Los_Angeles"))) |> 
...
isochrones_pop_pct = isochrones_pop_joined |> 
  ...
  summarise(pop = sum(POPULATION),
            pop_pct = pop/sum(nhgis_la_shp_subset$POPULATION), .groups = "drop") |> 
  mutate(arrival_time_ts = ymd_hms(arrival_time, tz = "America/Los_Angeles"),
         weekday = wday(arrival_time_ts, label = T),
         time_of_day = factor(hour(arrival_time_ts)))
...
```{r}
isochrones_pop_pct |> 
  select(venue, arrival_time_ts, weekday, time_of_day, travel_time_mins, pop, pop_pct) |> 
  arrange(venue, travel_time_mins, time_of_day, weekday) |> 
  View("Isochrones Sorted")
```
...
isochrones_pop_pct |> 
  select(venue, arrival_time_ts, weekday, time_of_day, travel_time_mins, pop, pop_pct) |> 
  arrange(venue, travel_time_mins, time_of_day, weekday) |> 
  pivot_wider(id_cols = c(venue, time_of_day, travel_time_mins, pop), names_from = weekday, values_from = pop_pct) |> 
  View("Isochrones Pivoted")
```
...
```{r}
isochrone_legend_colour = scale_colour_brewer(name = "Mins", type = "div", palette = 8, direction = -1, 
                        guide = guide_legend(direction = "horizontal", position = "bottom",
                                               theme = theme(legend.key.width = unit(.75, "cm"), legend.text.position = "bottom",
                                                             legend.key.height = unit(.5, "cm"), legend.title.position = "right")))
```
...
Plot population percentage for each venue by weekday and time of day

```{r}
isochrones_pop_pct |> 
  split(isochrones_pop_pct$venue) |> 
  map(\(df){
  ggplot(df, aes(x = travel_time_mins, y = pop_pct, colour = time_of_day))+
  geom_point()+
      geom_line()+
      # geom_col(aes(x = weekday, y = pop_pct, fill = time_of_day))+
      labs(title = str_c("How accessible is ", unique(df$venue), " by public transit?"),
           x = "Travel Time (mins)", y = "% of LA Population")+
      scale_fill_viridis_d()+
      scale_x_continuous(breaks = c(30, 60, 90, 120), labels = c("< 30", "30–60", "60–90", "90–120"), expand = expansion(add = .5))+
      scale_y_continuous(labels = scales::percent)+
  # facet_grid(rows = vars(travel_time_mins), cols = vars(time_of_day), scales = "free_y")+
      # facet_wrap(~travel_time_mins, ncol=1, scales = "free_y")+
  theme_minimal()
  })
```

Weekday doesn't have any effect on the isochrones, but time of day does.
Interestingly, LA Live is more accessible within 90 mins at 9am than 1pm, but more accessible within 30 mins at 1pm than 9am.
...
```{r}
isochrones_pop_pct |> 
  group_by(venue, travel_time_mins) |> 
  summarise(
    min = min(pop_pct),
    mean = mean(pop_pct),
  ...
  mutate(across(c(min:sd), \(x) round(x, 4)))
```
...
isochrones_pop_pct |> 
  ...
  ggplot(aes(x = pop_pct, y = time_of_day))+
  geom_boxplot(aes(x = pop_pct, fill = travel_time_fct), show.legend=F)+
  geom_point(aes())+
  stat_summary(fun = "median", geom = "label", mapping=aes(label = scales::percent(after_stat(x), accuracy = 1)), show.legend = F)+
  ...
  labs(title = "How accessible are Olympic venues by transit?",
       ...
       x = NULL, y = NULL)+
  facet_wrap(~travel_time_fct, ncol=1, strip.position = "left", labeller = label_wrap_gen(width = 20),  )+
...
travel_time_stacked = isochrones_pop_pct |> 
  filter(travel_time_mins <= 120, weekday == "Tue") |> 
  group_by(venue) |> 
  mutate(max_pct = max(pop_pct, na.rm=T)) |> 
  ungroup() |> 
...
```{r}
isochrones_pop_pct |> 
  select(venue, travel_time, arrival_time, time_of_day, pop, pop_pct) |> 
